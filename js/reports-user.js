document.addEventListener('DOMContentLoaded', () => {
  ...
  auth.onAuthStateChanged(async (user) => {
    if (!user) return;
    const snap = await db.collection('tasks').where('assignedTo','==', user.email).get();
    ...
    exportBtn?.addEventListener('click', () => {
      const header = ['Title','Priority','Status','Deadline','CreatedAt','UpdatedAt','Description'];
      ...
      a.download = `taskmate-my-report-${new Date().toISOString().slice(0,10)}.csv`;
      a.click();
    });
  });
});
